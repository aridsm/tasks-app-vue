<template>
  <section>
    <div class="flex justify-between items-center mb-6">
      ...
      >
      <btn-add @click="openFormNewDirectory">Adicionar diretório</btn-add>
    </div>
    <ul v-if="directoryStore.directories.length" class="flex rounded-sm overflow-auto gap-1 pb-2">
      <li
        ...
        :class="{
          'bg-blue-light dark:bg-lilac text-white': directoryStore.selectedDirectory?.id === directory.id,
          'bg-light-100 dark:bg-dark-200': directoryStore.selectedDirectory?.id !== directory.id
        }"
        tabindex="0"
        @click="directoryStore.selectDirectoryHandler(directory)"
      >
        <span class="max-w-60">{{ directory.name }} ({{ directory.count }})</span>
  ...
  >
    <input-text v-model="form.name" name="name" label="Nome" />
    <input-textarea
    ...
    <div class="flex mt-4 gap-4">
      <btn v-if="form.id" color="red" flat class="flex gap-3 items-center" @click="deleteDirectoryHandler">Excluir <icon icon="fa-regular fa-trash-can" class="text-lg" /></btn>
      <btn color="base" flat @click="() => (modalDirectoryOpen = false)" class="ml-auto px-4">Cancelar</btn>
      <btn @click="saveDirectoryHandler">
